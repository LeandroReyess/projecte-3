...
🧠 Objectius
Dissenyar i implementar una solució d’emmagatzematge flexible i redundant amb LVM a Linux (Zorin OS).
Dissenyar i implementar una solució equivalent amb Espais d’emmagatzematge a Windows 11.
Documentar detalladament els passos realitzats, configuracions i proves de funcionament.
Presentar una proposta tècnica per al client amb les conclusions i avantatges de cada tecnologia.
...
Afegir dos nous discos de 10 GB al grup de volums.
Crear un volum nou lvm_dades, formatar-lo i muntar-lo.
Afegir fitxers (per exemple, imatges descarregades).
Crear un snapshot (lv_snapshot) i demostrar el procés de restauració en cas de corrupció de dades.
Escalabilitat:
Demostrar l’ampliació del volum lv_dades aprofitant l’espai lliure del grup de volums.

📄 Documentació
➡️ Documentació LVM (Linux)
...
📄 Documentació
➡️ Documentació Espais d’emmagatzematge (Windows)
...
Buscarà i provarà les comandes i configuracions necessàries.
Documentarà el procés individualment.
Revisarà conjuntament la documentació final.
Pujarà la seva versió al repositori dins la carpeta tasca03/.
